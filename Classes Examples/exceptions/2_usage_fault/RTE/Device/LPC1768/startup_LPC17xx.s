...
Reset_Handler   PROC
				...
				;check they are enabled at: pewripherals -> core -> nested vector icnterrupt controller
				str r2, [r1]
				...
				; BUT IF YOU UPLOAD THIS CODE TO THE BOARD THE USAGE FAULT WILL BE RAISED BYT THE BOARD!!!!!!
				; ALWAYS TEST ALSO ON THE BOARD!!!!
				; THE BOARD RAISES AN USAGE FAULT !!!!
				; in the real boiard registers are not initialized to zero when you upload the code
				...
				;first I enable faults duye to div by zero
				; .load cconfig ontrol reg
				ldr r5, =0xE000ED14
				ldr r8, [r5]
...
HardFault_Handler\
				...
				;if the result of the test is not zero, it means bit 30 is zero
				;if the result of the test is zero, it means bit 30 is 1
