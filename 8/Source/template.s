...
SVC_Handler     PROC
				...
				; your code here
				; extract the 8 bit message
				AND r0, r0, #0xFF
				MOV r2, #0 ; encoded message
				MOV r3, #0 ; bit counter
				MOV r4, #0 ; bit counter for copies, from 0 to 23
...
loop			LSR r5, r0, r3
				...
				STMFD SP, {r1, r2} ; we will retrieve these after the line of the template
				LDMFD SP!, {R0-R12, LR} ; i'm keeping this line only because it is from the template
				
				LDR r1, [SP, #-16*4]
				LDR r2, [SP, #-15*4]
				
				
				; we rewrite the r0 that was auto-saved during handler entry
				TST LR, #4
				
				ITE EQ
				; We save in the MSP. We add 14, because r1 was saved after the MSP was modified by STMFD in the template...
				STREQ r2, [r1, #14*4] 
				STRNE r2, [r1]
