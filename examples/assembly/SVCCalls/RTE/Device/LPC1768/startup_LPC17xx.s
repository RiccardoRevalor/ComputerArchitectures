...
Reset_Handler   PROC
				...
				orr r0, r0, #2 ; set the 2nd bit: use PSP instead of MSP
				...
				ORR r0, r0, #1
				;BEWARE: ORR CONTROL, CONTROL, #1 raises an error because normal instructions do not work on special registers!
				...
				;svc call the svc code handler
				svc #7 ; have a look at supervisor code handler
...
SVC_Handler     PROC
				...
				;but in a complex system we have hundreds of svc so we have to retrieve the number
				;retrieve number 7
...
implemetation_of_SVC7
				; I wanna move to the privileged level for when I finish the execution of the handler and return to main
				...
				bic r0, r0, #1 ;also BIC is useful for clearing bits in the middle (it's not so immediate by using and and)
